...
namespace OnlyInvalid.ProcGenBuilding.Floor
{
    ...
    public class FloorEditor : Editor
    {
        ...
        private void Build()
        {
            ...
            Wall.Wall[] walls = new Wall.Wall[controlPoints.Length];
            ...
            for (int i = 0; i < startEndPoints.Count - 1; i += 2)
            {
                ...
                wall.GetComponent<Wall.Wall>().Initialize(data).Build();
                walls[count] = wall.GetComponent<Wall.Wall>();
                count++;
            }
