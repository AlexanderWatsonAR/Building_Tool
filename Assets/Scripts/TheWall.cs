...
{
    [SerializeField] private Vector3[] m_Corners; // control points.
    ...
    public void Init(IEnumerable<Vector3> points, float depth, List<Vector3> holePoints, bool flipFace = false)
    {
        m_Corners = points.ToArray();
        m_Depth = depth;
        m_FlipFace = flipFace;
        m_Holes = holePoints;
    }
    ...
    {
        if (m_Holes == null || m_Corners == null)
            return;
        ...
        {
            Handles.Label(m_Corners[i], i.ToString(), style);
        }
