...
public class WallSectionData : IData
{
    ...
    [SerializeField] private FrameData m_DoorFrameData;
    ...
    public FrameData DoorFrameData { get { return m_DoorFrameData; } set { m_DoorFrameData = value; } }
    ...
    public WallSectionData(WallElement wallElement, Vector3[] controlPoints, Vector2Int id, float depth, Vector3 normal) : this(wallElement, controlPoints, id, depth, normal, new WindowOpeningData(), new DoorwayData(), new ArchwayData(), new ExtensionData(), new WindowData(), new DoorData(), new DoorData(), new FrameData())
    {

    }

    public WallSectionData(WallElement wallElement, Vector3[] controlPoints, Vector2Int id, float depth, Vector3 normal, WindowOpeningData windowOpeningData, DoorwayData doorwayData, ArchwayData archwayData, ExtensionData extensionData, WindowData windowData, DoorData doorData, DoorData archDoorData, FrameData doorFrameData)
    {
    ...
    public WallSectionData(WallSectionData data) : this(data.WallElement, data.ControlPoints, data.ID, data.WallDepth, data.Normal, data.WindowOpening, data.Doorway, data.Archway, data.Extension, data.WindowData, data.DoorData, data.ArchDoorData, data.DoorFrameData)
    {
    }
}
